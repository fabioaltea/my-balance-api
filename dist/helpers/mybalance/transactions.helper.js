...
class TransactionsHelper {
    ...
    static formatDateOnly(dt) {
        return `${this.pad(dt.getDate())}-${this.pad(dt.getMonth() + 1)}-${dt.getFullYear()}`;
    }
    static formatDateTime(dt) {
        return `${dt.getFullYear()}-${this.pad(dt.getMonth() + 1)}-${this.pad(dt.getDate())} ${this.pad(dt.getHours())}:${this.pad(dt.getMinutes())}`;
    }
    ...
    static normalizeType(type, amount) {
    ...
    static normalizeMovementDate(input) {
        if (!input)
    ...
    static normalizeMetaDate(input) {
        if (input === null || input === undefined)
            return "";
        const raw = String(input).trim();
        if (raw === "")
            return ""; // stringa vuota rimane vuota
        if (this.DATE_TIME_REGEX.test(raw))
            return raw; // già ok
    ...
    static parseAmountToNumber(amount) {
    ...
    static groupTransactionsByMovement(transactions) {
    ...
    static updateMovement(authClientClient, spreadsheetId, movementRequest) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (updateData.length > 0) {
                results.push(yield google_1.GoogleHelper.update(authClientClient, spreadsheetId, updateData));
            }
            // Appendi nuove transactions
            if (newTransactions.length > 0) {
                const values = newTransactions.map((t) => this.transactionToRowValidated(t));
                const body = { values };
                results.push(yield google_1.GoogleHelper.append(authClientClient, spreadsheetId, SHEET_RANGE, body));
            }
            return results;
        });
    }
    ...
    static deleteMovement(authClient, spreadsheetId, movementId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            return yield google_1.GoogleHelper.update(authClient, spreadsheetId, updateData);
        });
    }
    // Utility: genera un movementId univoco
    static generateMovementId() {
    ...
    static transactionToRowValidated(t) {
        // Build row based on COLS mapping
        const row = new Array(16).fill(""); // 16 colonne
     ...
     */
    static updateTransactionsAccountName(authClient, spreadsheetId, oldAccountName, newAccountName) {
        return __awaiter(this, void 0, void 0, function* () {
            const rows = yield google_1.GoogleHelper.get(authClient, spreadsheetId, SHEET_RANGE);
            if (!rows || rows.length === 0)
                return 0;
     ...
     */
    static parseDateToTimestamp(dateStr) {
        if (!dateStr)
            return 0;
     ...
     */
    static getTransactionsSummary(authClient, spreadsheetId, startDate, endDate) {
        return __awaiter(this, void 0, void 0, function* () {
            // Get all transactions
            const allTransactions = yield this.listTransactions(authClient, spreadsheetId);
            // Apply date filtering if provided
            let transactions = allTransactions;
            if (startDate || endDate) {
                transactions = this.filterTransactionsByDateRange(allTransactions, startDate, endDate);
            }
            ...
            transactions.forEach((t) => {
                ...
                const status = (_a = t.status) === null || _a === void 0 ? void 0 : _a.toLowerCase();
...
TransactionsHelper.DATE_TIME_REGEX = /^\d{4}-\d{2}-\d{2} \d{2}:\d{2}$/; // meta dates
TransactionsHelper.DATE_ONLY_REGEX = /^\d{2}-\d{2}-\d{4}$/; // movement date
