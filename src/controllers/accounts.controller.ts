...
export class AccountsController {
  ...
  public static async getAccounts(req: any, res: Response): Promise<void> {
    try {
      ...
      const accounts = await GoogleAuthHelper.executeWithRetry(
        userEmail,
        deviceType,
        async (client) => AccountsHelper.getAccounts(spreadsheetId, client, calculateBalance),
      );
  ...
  public static async updateAccount(req: any, res: Response): Promise<void> {
    try {
      ...
      const updatedAccount = await GoogleAuthHelper.executeWithRetry(
        ...
        deviceType,
        async (client) =>
          AccountsHelper.updateAccount(spreadsheetId, client, accountId, updateData),
      );
   ...
   */
  public static async createAccountsBatch(req: any, res: Response): Promise<void> {
    try {
      const userEmail = req.userId;
      ...
      const createdAccounts = await GoogleAuthHelper.executeWithRetry(
        userEmail,
        deviceType,
        async (client) => AccountsHelper.createAccountsBatch(spreadsheetId, client, accounts),
      );
