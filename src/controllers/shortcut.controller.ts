...
export class ShortcutController {
   ...
   */
  static async createMovementViaShortcut(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await GoogleAuthHelper.executeWithRetry(
        user.email,
        deviceType,
        async (client) => TransactionsHelper.appendMovement(client, spreadsheetId, movement),
      );
   ...
   */
  private static findBestAccountMatch(inputAccount: string, accounts: IAccount[]): string {
    ...
    const partialMatch = accounts.find(
      (a) => a.name.toLowerCase().includes(input) || input.includes(a.name.toLowerCase()),
    );
