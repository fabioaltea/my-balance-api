...
export class SpreadsheetsHelper {
  ...
  static async createSpreadsheet(
    ...
    try {
      ...
      const res = await GoogleAuthHelper.executeWithRetry(userEmail, deviceType, async (client) =>
        GoogleHelper.create(client, userEmail),
      );
  ...
  static async initializeSpreadsheet(
    ...
    try {
      ...
      await GoogleAuthHelper.executeWithRetry(userEmail, deviceType, async (client) =>
        GoogleHelper.update(client, spreadsheetId, [...headerData, ...instructionsData]),
      );
      ...
      await GoogleAuthHelper.executeWithRetry(userEmail, deviceType, async (client) =>
        GoogleHelper.batchUpdateSpreadsheet(client, spreadsheetId, formatRequests),
      );
